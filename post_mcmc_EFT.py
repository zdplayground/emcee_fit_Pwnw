...
from post_lnprob_module_rsd_EFT import match_params, cal_pk_model, lnprior
...
def Sigma2_sd_integrand(k, tck_Pk_linw, R_bao, Sigma2_sm):
    Pk_lin_0 = interpolate.splev(k, tck_Pk_linw, der=0)
    Sm_kernel = np.exp(-0.25*k*k * Sigma2_sm)
    x1 = 0.5*(Sm_kernel**2.0 + (1.0-Sm_kernel)**2.0)
    x2 = np.sin(k*R_bao)/(k*R_bao) *(1.0-Sm_kernel)*Sm_kernel
    return Pk_lin_0 * (x1 - x2)

def Sigma2_ss_integrand(k, tck_Pk_linw, R_bao, Sigma2_sm):
    Pk_lin_0 = interpolate.splev(k, tck_Pk_linw, der=0)
    Sm_kernel = np.exp(-0.25*k*k * Sigma2_sm)
    return Pk_lin_0 * (1.0-np.sin(k*R_bao)/(k*R_bao))*Sm_kernel**2.0


# input Pk_obs = \hat{P}_wig - \hat{P}_now; no damping term for finger-of-god effect.
def lnlike(theta, params_indices, fix_params, k_p, mu_p, Pk_obs, ivar, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const):
    alpha_1, alpha_2, Sigma2_sm, Sigma2_dd, Sigma2_sd, Sigma2_ss, f, b_1, b_partial = match_params(theta, params_indices, fix_params)
    ...
    # calculate Sigma2_dd, Sigma2_sd and Sigma2_ss given by Zvonimir, normalization factor comes from linear power spectrum.
    Sigma2_dd = const * interpolate.splev(Sigma2_sm, tck_Sigma2_dd, der=0)
    Sigma2_sd = const * interpolate.splev(Sigma2_sm, tck_Sigma2_sd, der=0)
    Sigma2_ss = const * interpolate.splev(Sigma2_sm, tck_Sigma2_ss, der=0)

    Pk_model = cal_pk_model(Pk_linw, Pk_sm, k_t, mu_t, Sigma2_sm, Sigma2_dd, Sigma2_sd, Sigma2_ss, f, b_1, b_partial, norm_gf)
    diff = Pk_model - Pk_obs
    return -0.5* np.sum(diff**2.0 *ivar)

def lnprob(theta, params_indices, fix_params, k_p, mu_p, Pk_obs, ivar, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const):
    lp = lnprior(theta, params_indices, fix_params)
    if (lp < -1.e20):
        return -np.inf
    return lp + lnlike(theta, params_indices, fix_params, k_p, mu_p, Pk_obs, ivar, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const)

def chi2(*args):
    return -2 * lnlike(*args)

# MCMC routine
def mcmc_routine(ndim, N_walkers, theta, params_T, params_indices, fix_params, k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const, params_name, pool):
    ti = time.time()
    ...
    result = op.minimize(chi2, theta, args=(params_indices, fix_params, k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const), method='Powell')
    ...
    for jj in range(Nchains):
        ...
        sampler.append(emcee.EnsembleSampler(N_walkers, ndim, lnprob, a=2.0, args=(params_indices, fix_params, k_range, mu_range, Pk_wnow_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const), pool=pool))
    print(type(sampler))
...
def main():
    ...
    #const = 1.0
    Sigma2_dd_array = np.array([integrate.quad(Sigma2_dd_integrand, 1.05e-5, 100.0, args=(tck_Pk_linw, q_max, Sigma2_sm), epsabs=1.e-4, epsrel=1.e-4)[0] for Sigma2_sm in Sigma2_sm_array])
    Sigma2_sd_array = np.array([integrate.quad(Sigma2_sd_integrand, 1.05e-5, 100.0, args=(tck_Pk_linw, q_max, Sigma2_sm), epsabs=1.e-4, epsrel=1.e-4)[0] for Sigma2_sm in Sigma2_sm_array])
    Sigma2_ss_array = np.array([integrate.quad(Sigma2_ss_integrand, 1.05e-5, 100.0, args=(tck_Pk_linw, q_max, Sigma2_sm), epsabs=1.e-4, epsrel=1.e-4)[0] for Sigma2_sm in Sigma2_sm_array])
    ...
    params_mcmc = mcmc_routine(N_params, N_walkers, theta, params_T, params_indices, fix_params, k_p, mu_p, Pwnw_diff_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const, params_name, pool)
    print(params_mcmc)
    chi_square = chi2(params_mcmc[:, 0], params_indices, fix_params, k_p, mu_p, Pwnw_diff_obs, ivar_Pk_wnow, tck_Pk_linw, tck_Pk_sm, tck_Sigma2_dd, tck_Sigma2_sd, tck_Sigma2_ss, norm_gf, const)
